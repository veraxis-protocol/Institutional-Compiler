"""Single-use result-bearing execution harness for integration slice 001, RUN-002, v0.2.
...
EXECUTION_CANDIDATE_PATH: Final = (
    DOCS / "CDC-INTEGRATION-SLICE-001-RUN-002-EXECUTION-AUTHORIZATION-v0.2.CANDIDATE.json"
)
...
CRITERION_REQUIRED_FIELDS: Final[dict[str, tuple[str, ...]]] = {
    **{f"T-EARLY-0{n}": GROUP_REQUIRED_FIELDS["EARLY"] for n in range(1, 6)},
    ...
    "T-POS-06": (
        ...
        "write_order.issuance_authorization.timestamp_status",
        "write_order.attempt_record.timestamp",
        "write_order.reliance_record.timestamp",
        "write_order.observed_ordering_comparison.operands",
    ),
...
ISSUANCE_AUTHORIZATION_TIMESTAMP_STATUS: Final = (
    "ABSENT_IN_GOVERNED_RECORD__PROJECTION_IMPLEMENTATION_MISMATCH"
)


def _write_order_observation(run: dict[str, Any]) -> dict[str, Any]:
    """The three write moments, from the persisted governed records.

    Projection v0.1 §3.2 requires write-order timestamps for all three files.
    The attempt record carries ``claimed_at`` and the reliance record carries
    ``issued_at``, both archived here verbatim.  The issuance authorization file
    written by the frozen pipeline carries no timestamp field at all, so none is
    invented: the absence is named, and reported as a projection/implementation
    mismatch rather than papered over with a file mtime.
    """
    result = run["result"]
    ...
    authorization = (
        json.loads(authorization_path.read_bytes()) if authorization_path.is_file() else {}
    )
    claimed_at = attempt.get("claimed_at")
    issued_at = reliance.get("issued_at")
    ordered = None
    if claimed_at is not None and issued_at is not None:
        ordered = claimed_at <= issued_at
    return {
        "declared_order": ["issuance_authorization", "attempt_record", "reliance_record"],
        "issuance_authorization": {
            "path": str(authorization_path),
            "timestamp": None,
            "timestamp_status": ISSUANCE_AUTHORIZATION_TIMESTAMP_STATUS,
            "fields_present": sorted(authorization),
        },
        "attempt_record": {
            "timestamp_field": "claimed_at",
            "timestamp": claimed_at,
            "attempt_state": attempt.get("attempt_state"),
        },
        "reliance_record": {
            "timestamp_field": "issued_at",
            "timestamp": issued_at,
            "disposition": reliance.get("reliance_disposition"),
        },
        "observed_ordering_comparison": {
            "attempt_claimed_at_le_reliance_issued_at": ordered,
            "operands": {"claimed_at": claimed_at, "issued_at": issued_at},
            "issuance_authorization_comparable": False,
        },
    }
...
def execute_run() -> dict[str, Any]:
    ...
    # Structural conformance BEFORE the attempt is marked complete.  If it fails
    # the execution is frozen exactly as observed; it is not repaired or retried.
