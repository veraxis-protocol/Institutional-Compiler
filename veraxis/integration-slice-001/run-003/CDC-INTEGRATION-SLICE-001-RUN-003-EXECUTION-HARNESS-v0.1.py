"""Single-use result-bearing execution harness for integration slice 001, RUN-003.

Successor to RUN-002 harness v0.4 (``cff3f853…``), which is preserved and spent.
Every RUN-002 control is carried forward; one architectural defect is removed by
construction.

**What actually happened in RUN-002, stated correctly.** RUN-002 did not fail
because ``_scenario()`` refused. The refusal was correct — fail-closed resolution
is the behaviour the owner required, and it did exactly what it was built to do.
RUN-002 failed because the authoritative declaration consulted by *preparation*
was incomplete relative to what *executors could reach*: two sets, independently
maintained, that silently drifted. Six directories the authority-only cases needed
were never declared. Adding those six strings would have repaired the instance and
left the defect class untouched, so that is not what this harness does.

RUN-003 removes the divergence by construction. ``SCENARIO_MANIFEST`` is the sole
declaration of every permitted pipeline scenario root. Preparation iterates it.
Resolution goes through ``scenario_for(key)`` and nothing else — there is no path
by which an executor can name a directory the manifest does not contain, because
executors hand over a manifest key and never a string. The old
``criterion_id.lower().replace(...)`` derivation is gone: each criterion declares
its scenario dependency explicitly, or declares ``NONE``.

Three sets must be identical before a candidate may be frozen: what the manifest
declares, what executors can reach, and what preparation creates. A static control
proves it, negative controls prove the control itself fails when it should, and a
source inspection backs all of that up as defense in depth.

Modes: ``manifest``, ``scaffold``, ``preflight``, ``execute``. Only ``execute`` is
result-bearing, and only against a valid owner EXEC-003 issuance.
"""
...
CONTROL_ROOT: Final = Path("/private/tmp/cdc-is1-run003")
RUNTIME_ROOT: Final = Path("/private/tmp/cdc-integration-slice-001-run-003")
...
EXECUTION_ID: Final = "CDC-INTEGRATION-SLICE-001-RUN-003"
TRACE_ID: Final = "CDC-INTEGRATION-SLICE-001-RUN-003-TRACE"
AUTHORIZATION_ID: Final = "OWNER-AUTHORIZATION-INTEGRATION-SLICE-001-EXEC-003"

DOCS: Final = CONTROL_ROOT / "docs/operations"
EXECUTION_CANDIDATE_PATH: Final = (
    DOCS / "CDC-INTEGRATION-SLICE-001-RUN-003-EXECUTION-AUTHORIZATION-v0.1.CANDIDATE.json"
)
EXECUTION_ISSUANCE_PATH: Final = (
    DOCS / "CDC-INTEGRATION-SLICE-001-RUN-003-EXECUTION-AUTHORIZATION-ISSUANCE-001.json"
)
...
def malformed_observation_control() -> dict[str, Any]:
    ...
    return {
        "record_class": "CDC_INTEGRATION_SLICE_001_RUN_003_MALFORMED_OBSERVATION_CONTROL",
        "fixture_class": "CONTROL_FIXTURE_ONLY",
        "result_bearing": False,
...
def validate_observation_set(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "record_class": "CDC_INTEGRATION_SLICE_001_RUN_003_OBSERVATION_CONFORMANCE",
...
# ---------------------------------------------------------------------------
# The canonical scenario manifest — the sole declaration of scenario roots
# ---------------------------------------------------------------------------
#
# RUN-002 kept one list for preparation and an implicit second set encoded in
# executor call sites.  They drifted, and the drift was invisible until an
# ordinal had already been spent.  There is now exactly one authoritative mapping
# from scenario identity to relative path; preparation iterates it and resolution
# consults it, so the two cannot disagree.

SCENARIO_MANIFEST: Final[dict[str, str]] = {
    "POSITIVE": "pipeline/positive",
    "TOCTOU_CURRENTNESS": "pipeline/toctou-currentness",
    "TOCTOU_AUTHORITY": "pipeline/toctou-authority",
    "HISTORICAL": "pipeline/historical",
    "CASE_A": "pipeline/case-a",
    "CASE_B": "pipeline/case-b",
    "CASE_C": "pipeline/case-c",
    "CASE_D": "pipeline/case-d",
    "CASE_E": "pipeline/case-e",
    "CASE_F": "pipeline/case-f",
    "CASE_G": "pipeline/case-g",
    "CASE_H": "pipeline/case-h",
    "CASE_I": "pipeline/case-i",
    "CASE_J": "pipeline/case-j",
    "CASE_M": "pipeline/case-m",
    "CASE_N": "pipeline/case-n",
    "CASE_O": "pipeline/case-o",
    "CASE_Q": "pipeline/case-q",
    "CASE_R": "pipeline/case-r",
    "CASE_S": "pipeline/case-s",
    "DIG_04": "pipeline/dig-04",
    "DIG_05": "pipeline/dig-05",
    "DIG_06": "pipeline/dig-06",
}
INFRASTRUCTURE_ONLY_KEYS: Final[frozenset[str]] = frozenset()

# Output containers.  Not scenario roots, never resolvable through scenario_for,
# and deliberately outside the manifest's identity.
OUTPUT_DIRECTORIES: Final = ("observations", "raw", "accounting")

NO_SCENARIO: Final = "NONE"

# Every criterion declares its scenario dependency explicitly.  Nothing is
# derived from the criterion id at runtime.
CRITERION_SCENARIO_DEPENDENCY: Final[dict[str, str]] = {
    **{f"T-EARLY-0{n}": NO_SCENARIO for n in range(1, 6)},
    **{f"T-POS-0{n}": "POSITIVE" for n in range(1, 7)},
    "T-CASE-A": "CASE_A",
    "T-CASE-B": "CASE_B",
    "T-CASE-C": "CASE_C",
    "T-CASE-D": "CASE_D",
    ...
    "T-CASE-I": "CASE_I",
    "T-CASE-J": "CASE_J",
    "T-CASE-K": "TOCTOU_CURRENTNESS",
    "T-CASE-L": "HISTORICAL",
    "T-CASE-M": "CASE_M",
    "T-CASE-N": "CASE_N",
    "T-CASE-O": "CASE_O",
    "T-CASE-P": "TOCTOU_AUTHORITY",
    "T-CASE-Q": "CASE_Q",
    "T-CASE-R": "CASE_R",
    "T-CASE-S": "CASE_S",
    "T-DIG-01": NO_SCENARIO,
    "T-DIG-02": NO_SCENARIO,
    "T-DIG-03": NO_SCENARIO,
    "T-DIG-04": "DIG_04",
    "T-DIG-05": "DIG_05",
    "T-DIG-06": "DIG_06",
    "T-DIG-07": NO_SCENARIO,
    "T-DIG-08": NO_SCENARIO,
    "T-EPOCH-A": NO_SCENARIO,
    "T-EPOCH-B": NO_SCENARIO,
    "T-EPOCH-C": NO_SCENARIO,
}
...
RUNNER_IDENTITY: Final = "CDC-INTEGRATION-SLICE-001-RUN-003-EXECUTION-HARNESS-v0.1"
...
def prepare_scenario_tree(root: Path) -> list[str]:
    """Create every manifest scenario root, plus the output containers.

    The manifest is the only source of scenario roots here.  Nothing else creates
    a scenario directory, and this function invents none of its own.
    """
    created: list[str] = []
    for relative in (*SCENARIO_MANIFEST.values(), *OUTPUT_DIRECTORIES):
        path = root / relative
...
def prepared_scenario_set(root: Path) -> set[str]:
    """The scenario roots that actually exist under a prepared root."""
    pipeline = root / "pipeline"
    if not pipeline.is_dir():
        return set()
    return {
        str(path.relative_to(root))
        for path in pipeline.iterdir()
        if path.is_dir()
    }


def scenario_for(key: str) -> Path:
    """Resolve a scenario directory through the manifest, and only the manifest.

    An executor hands over a manifest key.  There is no parameter through which it
    can name a path, so the RUN-002 divergence — a reachable directory the
    declaration never knew about — cannot recur: an undeclared key fails here,
    before anything is created or written.
    """
    if key not in SCENARIO_MANIFEST:
        raise HarnessRefusalError(
            f"scenario key not in the canonical manifest: {key!r}; executors resolve "
            "scenario roots only through SCENARIO_MANIFEST"
        )
    path = RUNTIME_ROOT / SCENARIO_MANIFEST[key]
    if not path.is_dir():
        raise HarnessRefusalError(
            f"manifest scenario directory absent: {path}; prepare_scenario_tree() must "
            "create every manifest entry before any scenario writer runs"
        )
    return path


def scenario_for_criterion(criterion_id: str) -> Path:
    """Resolve the scenario a criterion declared.  No id-to-path derivation."""
    key = CRITERION_SCENARIO_DEPENDENCY[criterion_id]
    if key == NO_SCENARIO:
        raise HarnessRefusalError(
            f"{criterion_id} declares no scenario dependency and must not resolve one"
        )
    return scenario_for(key)
...
def scaffold_control(root: Path | None = None) -> dict[str, Any]:
    ...
    return {
        "record_class": "CDC_INTEGRATION_SLICE_001_RUN_003_SCAFFOLD_CONTROL",
        "scaffold_root": str(target),
        "scaffold_root_abandoned": root is None,
        "directories_expected": len(SCENARIO_MANIFEST) + len(OUTPUT_DIRECTORIES),
...
def scenario_refusal_control() -> dict[str, Any]:
    ...
    try:
        ...
        omitted = probe / SCENARIO_MANIFEST["POSITIVE"]
        shutil.rmtree(omitted)
        refused = False
        message = None
        try:
            scenario_for("POSITIVE")
        except HarnessRefusalError as error:
            refused = True
            message = str(error)
        resolved_present = scenario_for("TOCTOU_CURRENTNESS").is_dir()
    finally:
        RUNTIME_ROOT = original
        shutil.rmtree(probe, ignore_errors=True)
    after = _RESULT_BEARING_INVOCATIONS["count"]
    return {
        "record_class": "CDC_INTEGRATION_SLICE_001_RUN_003_SCENARIO_REFUSAL_CONTROL",
        "omitted_directory": SCENARIO_MANIFEST["POSITIVE"],
...
# ---------------------------------------------------------------------------
# Manifest controls — non-result-bearing
# ---------------------------------------------------------------------------


def manifest_identity() -> dict[str, Any]:
    """The canonical manifest's own immutable identity, before candidate freeze.

    A harness/control artifact identity, not a semantic digest class: it is the
    ordinary canonical serialization applied to the manifest object.
    """
    payload = {
        "record_class": "CDC_INTEGRATION_SLICE_001_RUN_003_SCENARIO_MANIFEST",
        "schema_version": "RUN-003-SCENARIO-MANIFEST-v0.1",
        "execution_id": EXECUTION_ID,
        "pipeline_scenario_roots": dict(SCENARIO_MANIFEST),
        "pipeline_scenario_count": len(SCENARIO_MANIFEST),
        "infrastructure_only_keys": sorted(INFRASTRUCTURE_ONLY_KEYS),
        "output_directories": list(OUTPUT_DIRECTORIES),
        "criterion_scenario_dependency": dict(CRITERION_SCENARIO_DEPENDENCY),
        "no_scenario_sentinel": NO_SCENARIO,
    }
    return {
        **payload,
        "manifest_digest": canonical_digest(payload),
        "canonical_bytes": len(canonical_bytes(payload)),
        "identity_class": "HARNESS_CONTROL_ARTIFACT_IDENTITY",
        "semantic_digest_class_added": False,
        "result_bearing": False,
    }


def executor_reachable_scenarios(
    dependencies: dict[str, str] | None = None,
) -> tuple[set[str], list[str]]:
    """Every scenario key an executor can reach, and any not in the manifest."""
    declared = dependencies if dependencies is not None else CRITERION_SCENARIO_DEPENDENCY
    keys = {key for key in declared.values() if key != NO_SCENARIO}
    undeclared = sorted(key for key in keys if key not in SCENARIO_MANIFEST)
    return keys, undeclared


def scenario_reachability_control(
    *,
    dependencies: dict[str, str] | None = None,
    prepared: set[str] | None = None,
) -> dict[str, Any]:
    """The three sets must be identical: manifest, reachable, prepared.

    This is the control RUN-002 lacked.  Preparation and reachability were two
    independently maintained sets there; here they are compared explicitly, and a
    candidate may not be frozen unless they agree exactly.
    """
    manifest_paths = set(SCENARIO_MANIFEST.values())
    reachable_keys, undeclared = executor_reachable_scenarios(dependencies)
    reachable_paths = {
        SCENARIO_MANIFEST[key] for key in reachable_keys if key in SCENARIO_MANIFEST
    }
    prepared_paths = set(prepared) if prepared is not None else set()
    unconsumed = sorted(
        key
        for key in SCENARIO_MANIFEST
        if key not in reachable_keys and key not in INFRASTRUCTURE_ONLY_KEYS
    )
    manifest_equals_reachable = not undeclared and manifest_paths == reachable_paths
    manifest_equals_prepared = manifest_paths == prepared_paths
    all_equal = manifest_equals_reachable and manifest_equals_prepared
    return {
        "record_class": "CDC_INTEGRATION_SLICE_001_RUN_003_SCENARIO_REACHABILITY_CONTROL",
        "manifest_pipeline_scenario_count": len(manifest_paths),
        "executor_reachable_pipeline_scenario_count": len(reachable_paths),
        "prepared_pipeline_scenario_count": len(prepared_paths),
        "manifest_equals_executor_reachability": manifest_equals_reachable,
        "manifest_equals_prepared_set": manifest_equals_prepared,
        "all_three_sets_equal": all_equal,
        "undeclared_executor_scenario_dependencies": undeclared,
        "manifest_entries_without_declared_consumer": unconsumed,
        "manifest_only": sorted(manifest_paths - prepared_paths),
        "prepared_only": sorted(prepared_paths - manifest_paths),
        "candidate_freeze_eligible": all_equal and not undeclared and not unconsumed,
        "result_bearing_invocations": 0,
        "result_bearing": False,
    }


CALL_SITE_ENTRYPOINTS: Final = ("scenario_for", "scenario_for_criterion")


def call_site_control() -> dict[str, Any]:
    """Defense in depth: inspect the source for any path back to a literal.

    The primary control is the manifest.  This one exists so a future edit that
    reintroduces a string-addressed scenario is caught statically rather than at
    the cost of an ordinal.
    """
    source = Path(__file__).read_text(encoding="utf-8")
    # Excise this function's own body before scanning: its search tokens are
    # literals, and a control that matches itself reports a defect that is only
    # its own text.
    # Scan code only: the module docstring names the removed pattern in prose,
    # and this function's own search tokens are literals.  Neither is a call site.
    code = source[source.index("from __future__ import annotations"):]
    marker = "def call_site" + "_control("
    start = code.index(marker)
    end = code.index("def manifest_negative" + "_controls(")
    body = code[:start] + code[end:]
    literal = "_scenario" + '("'
    literal_single = "_scenario" + "('"
    literal_calls = body.count(literal) + body.count(literal_single)
    derivations = sum(
        body.count(token)
        for token in ("criterion_id" + ".lower()", "cid" + ".lower()", ".replace(" + '"t-case-"')
    )
    return {
        "record_class": "CDC_INTEGRATION_SLICE_001_RUN_003_CALL_SITE_CONTROL",
        "direct_arbitrary_scenario_literal_calls": literal_calls,
        "dynamic_criterion_to_path_derivations": derivations,
        "result_bearing_scenario_resolution_functions": list(CALL_SITE_ENTRYPOINTS),
        "scenario_resolution_single_entrypoint": True,
        "defense_in_depth_only": True,
        "result_bearing": False,
    }


def manifest_negative_controls() -> dict[str, Any]:
    """Prove the reachability control fails when it should.

    Synthetic fixtures only.  No semantic implementation function executes, and
    no result-bearing route is touched.
    """
    manifest_paths = set(SCENARIO_MANIFEST.values())

    undeclared_key = scenario_reachability_control(
        dependencies={**CRITERION_SCENARIO_DEPENDENCY, "T-CONTROL-FIXTURE": "CASE_Z"},
        prepared=manifest_paths,
    )
    missing_preparation = scenario_reachability_control(
        prepared=manifest_paths - {SCENARIO_MANIFEST["CASE_A"]},
    )
    extra_preparation = scenario_reachability_control(
        prepared=manifest_paths | {"pipeline/case-undeclared"},
    )
    resolution_refusal = None
    try:
        scenario_for("CASE_Z")
    except HarnessRefusalError as error:
        resolution_refusal = str(error)

    rows = {
        "A_executor_references_undeclared_key": {
            "fixture_class": "CONTROL_FIXTURE_ONLY",
            "synthetic_dependency": "CASE_Z",
            "undeclared_executor_scenario_dependencies": undeclared_key[
                "undeclared_executor_scenario_dependencies"
            ],
            "reachability_control": (
                "FAIL" if not undeclared_key["candidate_freeze_eligible"] else "PASS"
            ),
            "candidate_freeze_eligible": undeclared_key["candidate_freeze_eligible"],
            "scenario_for_refused_undeclared_key": resolution_refusal is not None,
            "control": "PASS" if not undeclared_key["candidate_freeze_eligible"] else "FAIL",
        },
        "B_manifest_entry_not_prepared": {
            "fixture_class": "CONTROL_FIXTURE_ONLY",
            "omitted": SCENARIO_MANIFEST["CASE_A"],
            "set_equality": missing_preparation["all_three_sets_equal"],
            "candidate_freeze_eligible": missing_preparation["candidate_freeze_eligible"],
            "control": "PASS" if not missing_preparation["candidate_freeze_eligible"] else "FAIL",
        },
        "C_extra_prepared_directory": {
            "fixture_class": "CONTROL_FIXTURE_ONLY",
            "extra": "pipeline/case-undeclared",
            "set_equality": extra_preparation["all_three_sets_equal"],
            "candidate_freeze_eligible": extra_preparation["candidate_freeze_eligible"],
            "control": "PASS" if not extra_preparation["candidate_freeze_eligible"] else "FAIL",
        },
    }
    return {
        "record_class": "CDC_INTEGRATION_SLICE_001_RUN_003_MANIFEST_NEGATIVE_CONTROLS",
        "controls": rows,
        "controls_total": len(rows),
        "controls_passed": sum(1 for row in rows.values() if row["control"] == "PASS"),
        "semantic_functions_invoked": 0,
        "result_bearing": False,
    }


def manifest_preparation_control() -> dict[str, Any]:
    """Create and resolve all 23 roots in a throwaway root, then destroy it."""
    _wrap_result_bearing_routes()
    before = _RESULT_BEARING_INVOCATIONS["count"]
    probe = Path("/private/tmp") / f"cdc-integration-slice-001-run-003-manifest-{os.getpid()}"
    shutil.rmtree(probe, ignore_errors=True)
    global RUNTIME_ROOT  # noqa: PLW0603
    original = RUNTIME_ROOT
    rows = []
    ...
    try:
        RUNTIME_ROOT = probe
        prepare_scenario_tree(probe)
        for key, relative in SCENARIO_MANIFEST.items():
            path = probe / relative
            probe_file = path / ".writable-probe"
            try:
                ...
                writable = False
            resolved_path = scenario_for(key)
            ...
            rows.append(
                {
                    ...
                    "parent_exists": path.parent.is_dir(),
                    "writable": writable,
                    "resolved_equals_created": resolved_path == path,
                }
            )
        prepared = prepared_scenario_set(probe)
    finally:
        RUNTIME_ROOT = original
        shutil.rmtree(probe, ignore_errors=True)
    after = _RESULT_BEARING_INVOCATIONS["count"]
    created = sum(1 for row in rows if row["exists"] and row["parent_exists"] and row["writable"])
    return {
        "record_class": "CDC_INTEGRATION_SLICE_001_RUN_003_MANIFEST_PREPARATION_CONTROL",
        "throwaway_root": str(probe),
        "throwaway_root_destroyed": not probe.exists(),
        "expected": len(SCENARIO_MANIFEST),
        "created": created,
        "resolved": len(resolved),
        "resolved_equals_created": all(row["resolved_equals_created"] for row in rows),
        "prepared_scenario_set_size": len(prepared),
        "rows": rows,
        "result_bearing_invocations": after - before,
        "control": (
            "PASS"
            if created == len(SCENARIO_MANIFEST)
            and len(resolved) == len(SCENARIO_MANIFEST)
            and all(row["resolved_equals_created"] for row in rows)
            else "FAIL"
        ),
        "result_bearing": False,
    }
...
def authority_binding_control() -> dict[str, Any]:
    ...
    return {
        "record_class": "CDC_INTEGRATION_SLICE_001_RUN_003_AUTHORITY_BINDING_CONTROL",
...
def filesystem_timestamp_control() -> dict[str, Any]:
    ...
    stat_calls = [token for token in ("os.stat(", ".stat()", "getmtime", "getctime")
                  if token in body]
    return {
        "record_class": "CDC_INTEGRATION_SLICE_001_RUN_003_FILESYSTEM_TIMESTAMP_CONTROL",
...
def preflight() -> dict[str, Any]:
    ...
    preparation = manifest_preparation_control()
    reachability = scenario_reachability_control(
        prepared={row["relative_path"] for row in preparation["rows"] if row["exists"]}
    )
    call_sites = call_site_control()
    negatives = manifest_negative_controls()

    check("manifest_pipeline_scenario_count", 23, reachability["manifest_pipeline_scenario_count"])
    check("executor_reachable_pipeline_scenario_count", 23,
          reachability["executor_reachable_pipeline_scenario_count"])
    check("prepared_pipeline_scenario_count", 23,
          reachability["prepared_pipeline_scenario_count"])
    check("manifest_equals_executor_reachability", True,
          reachability["manifest_equals_executor_reachability"])
    check("manifest_equals_prepared_set", True, reachability["manifest_equals_prepared_set"])
    check("all_three_scenario_sets_equal", True, reachability["all_three_sets_equal"])
    check("undeclared_executor_scenarios", [],
          reachability["undeclared_executor_scenario_dependencies"])
    check("unconsumed_manifest_scenarios", [],
          reachability["manifest_entries_without_declared_consumer"])
    check("direct_arbitrary_scenario_literal_calls", 0,
          call_sites["direct_arbitrary_scenario_literal_calls"])
    check("dynamic_criterion_to_path_derivations", 0,
          call_sites["dynamic_criterion_to_path_derivations"])
    check("scenario_resolution_single_entrypoint", True,
          call_sites["scenario_resolution_single_entrypoint"])
    check("manifest_negative_undeclared_key_control", "PASS",
          negatives["controls"]["A_executor_references_undeclared_key"]["control"])
    check("manifest_negative_missing_preparation_control", "PASS",
          negatives["controls"]["B_manifest_entry_not_prepared"]["control"])
    check("manifest_negative_extra_preparation_control", "PASS",
          negatives["controls"]["C_extra_prepared_directory"]["control"])
    check("throwaway_manifest_creation", 23, preparation["created"])
    check("throwaway_manifest_resolution", 23, preparation["resolved"])
    check("throwaway_created_equals_resolved", True, preparation["resolved_equals_created"])
    check("criterion_scenario_dependency_count", CRITERIA_TOTAL,
          len(CRITERION_SCENARIO_DEPENDENCY))
    check("criterion_scenario_dependency_covers_universe", True,
          set(CRITERION_SCENARIO_DEPENDENCY) == set(FROZEN_CRITERION_ORDER))
    ...
    check("node_accounting_runner_identity_exact",
          "CDC-INTEGRATION-SLICE-001-RUN-003-EXECUTION-HARNESS-v0.1", RUNNER_IDENTITY)
    ...
    return {
        "record_class": "CDC_INTEGRATION_SLICE_001_RUN_003_PREFLIGHT_OBSERVATION",
        "execution_id": EXECUTION_ID,
        "observed_at": _now(),
        "checks": checks,
        ...
        "scenario_manifest_identity": identity,
        "scenario_reachability_control": reachability,
        "call_site_control": call_sites,
        "manifest_negative_controls": negatives,
        "manifest_preparation_control": preparation,
...
def attempt_record_path(issuance_digest: str) -> Path:
    ...
    return RUNTIME_ROOT / f".cdc-integration-slice-001-run-003-attempt-{issuance_digest}.json"
...
def consume_attempt(issuance_digest: str) -> dict[str, Any]:
    ...
    path = attempt_record_path(issuance_digest)
    record = {
        "record_class": "CDC_INTEGRATION_SLICE_001_RUN_003_EXECUTION_ATTEMPT",
        "authorization_id": AUTHORIZATION_ID,
...
def _positive_run() -> dict[str, Any]:
    ...
    if "run" not in _POSITIVE_CACHE:
        ...
        _POSITIVE_CACHE["run"] = _capture(
            "POSITIVE", module._run_pipeline(scenario_for("POSITIVE"))
        )
...
def _exec_pos_04(_: str) -> dict[str, Any]:
    ...
    scenario = scenario_for("POSITIVE")
    ...
    return {
        ...
        "evidence_refs": [
            ...
            "raw/RUN-003-PROCESS-BOUNDARY-OBSERVATION-v0.1.json",
...
def _exec_pos_05(_: str) -> dict[str, Any]:
    ...
    return {
        ...
        "evidence_refs": [
            ...
            "raw/RUN-003-CONSUMER-VALIDATION-OBSERVATION-v0.1.json",
...
def _authority_only_case(
    ...
    scenario = scenario_for_criterion(criterion_id)
...
def _consumer_case(
    ...
    **kwargs: Any,
    ...
    module = _pipeline()
    key = CRITERION_SCENARIO_DEPENDENCY[criterion_id]
    run = _capture(key, module._run_pipeline(scenario_for_criterion(criterion_id), **kwargs))
    result = run["result"]
    reliance = result["reliance_record"]
    observed = {
...
def _exec_case_j(criterion_id: str) -> dict[str, Any]:
    ...
    module = _pipeline()
    scenario = scenario_for_criterion("T-CASE-J")
    first = _capture("CASE_J_FIRST", module._run_pipeline(scenario, reliance_id="SYNTH-RELIANCE-J1"))
    second = _capture("CASE_J_REPLAY", module._run_pipeline(
...
def _exec_case_k(criterion_id: str) -> dict[str, Any]:
    ...
    module = _pipeline()
    fixtures = _fixtures()
    run = _capture("TOCTOU_CURRENTNESS", module._run_pipeline(
        scenario_for_criterion("T-CASE-K"),
...
def _exec_case_l(criterion_id: str) -> dict[str, Any]:
    ...
    module = _pipeline()
    fixtures = _fixtures()
    scenario = scenario_for_criterion("T-CASE-L")
    issued = _capture("HISTORICAL_ISSUED", module._run_pipeline(scenario, reliance_id="SYNTH-RELIANCE-L1"))
    issued_record = issued["result"]["reliance_record"]
    path = Path(issued["output_path"])
    before = file_sha256(path.read_bytes())
    later = _capture("HISTORICAL_LATER", module._run_pipeline(
        scenario,
...
def _exec_case_p(criterion_id: str) -> dict[str, Any]:
    ...
    module = _pipeline()
    fixtures = _fixtures()
    run = _capture("TOCTOU_AUTHORITY", module._run_pipeline(
        scenario_for_criterion("T-CASE-P"),
        producer_now=fixtures.T1,
        consumer_now=fixtures.T2,
...
def _register_digests() -> None:
    ...
    def dig_04(cid: str) -> dict[str, Any]:
        module = _pipeline()
        run = _capture("DIG_04", module._run_pipeline(scenario_for_criterion(cid)))
        _install_paths()
        from oic.cdc_propagation import envelope_digest
        ...
        return _dig_observation(
            ...
            digested_object_reference="DIG_04 scenario envelope minus its own digest",
            canonical_byte_count=len(
    ...
    def dig_05(cid: str) -> dict[str, Any]:
        module = _pipeline()
        run = _capture("DIG_05", module._run_pipeline(scenario_for_criterion(cid)))
        _install_paths()
        ...
        return _dig_observation(
            ...
            digested_object_reference="DIG_05 scenario consumer validation, checks[] 1..16",
            canonical_byte_count=len(
    ...
    def dig_06(cid: str) -> dict[str, Any]:
        module = _pipeline()
        run = _capture("DIG_06", module._run_pipeline(scenario_for_criterion(cid)))
        _install_paths()
        ...
        return _dig_observation(
            ...
            digested_object_reference="DIG_06 scenario reliance record minus its own digest",
            canonical_byte_count=len(
                canonical_bytes({k: v for k, v in record.items()
...
def _view(name: str, body: dict[str, Any]) -> dict[str, Any]:
    return {
        "record_class": f"CDC_INTEGRATION_SLICE_001_RUN_003_{name}",
        "execution_id": EXECUTION_ID,
        "trace_id": TRACE_ID,
...
def runner_accounting(observations: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "record_class": "CDC_INTEGRATION_SLICE_001_RUN_003_RUNNER_ACCOUNTING",
        "schema_version": "RUN-003-RUNNER-ACCOUNTING-v0.1",
        "execution_id": EXECUTION_ID,
        "trace_id": TRACE_ID,
        "NON_LOAD_BEARING": True,
        "non_load_bearing": True,
...
def execute_run() -> dict[str, Any]:
    ...
    def refuse(reason: str, **extra: Any) -> dict[str, Any]:  # noqa: ANN401
        return {
            "record_class": "CDC_INTEGRATION_SLICE_001_RUN_003_EXECUTION_REFUSAL",
    ...
    if not eligible:
        return {
            "record_class": "CDC_INTEGRATION_SLICE_001_RUN_003_RESULT",
...
def persist(result: dict[str, Any]) -> dict[str, Any]:
    ...
    view_members = [
        write(raw_dir / f"RUN-003-{name}-v0.1.json", body)
        for name, body in result["raw_views"].items()
    ]
    accounting_member = write(
        accounting_dir / "RUN-003-RUNNER-ACCOUNTING-v0.1.json", result["runner_accounting"]
    )
    conformance_member = write(
        raw_dir / "RUN-003-OBSERVATION-CONFORMANCE-v0.1.json", result["observation_conformance"]
    )
    ...
    ledger = {
        ...
        "pytest_accounting_ref": "accounting/RUN-003-RUNNER-ACCOUNTING-v0.1.json",
        "ledger_digest": "",
    }
    ledger["ledger_digest"] = criterion_ledger_digest(ledger)
    ledger_member = write(
        RUNTIME_ROOT / "CDC-INTEGRATION-SLICE-001-RUN-003-CRITERION-EVIDENCE-LEDGER-v0.1.json",
        ledger,
    )
    ...
    package = {
        "record_class": "CDC_INTEGRATION_SLICE_001_RUN_003_RAW_EXECUTION_PACKAGE",
        "schema_version": "CDC-INTEGRATION-SLICE-001-RUN-003-RAW-EXECUTION-PACKAGE-v0.1",
        "execution_id": EXECUTION_ID,
        "trace_id": TRACE_ID,
        "authorization_id": AUTHORIZATION_ID,
    ...
    package_member = write(
        RUNTIME_ROOT / "CDC-INTEGRATION-SLICE-001-RUN-003-RAW-EXECUTION-PACKAGE-v0.1.json",
        package,
    )
    return {
...
if __name__ == "__main__":
    ...
    if mode == "manifest":
        print(json.dumps(manifest_identity(), indent=2, sort_keys=True, default=str))
    elif mode == "scaffold":
