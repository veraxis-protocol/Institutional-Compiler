# INTEGRATION SLICE 001 — DIGEST DERIVATION v0.5

```
supersedes      INTEGRATION-SLICE-001-DIGEST-DERIVATION-v0.4.md
                sha256 494c91ace109ba050c40b72cc2f0f1cc64366386376212d50edbb3b9a418d1e7
                8950 B — preserved unchanged as a historical artifact
companions      SEMANTIC-DESIGN-v0.4          03ca22e9…  unchanged by this document
                CRITERION-EVIDENCE-PROJECTION-v0.2  00af89d21fda41adaf4a95d5938d3b6fc90666d4ebf902b9c205aa42e7974db2
status          FROZEN BEFORE IMPLEMENTATION
result_bearing  false
```

**Sole change from v0.4:** Classes 9 and 10 are added. Classes 1–8, the canonical
rule, the persisted-file identity rules and every previously published vector are
...
```
1 currentness_epoch_digest        as-of projection; complete inputs and vectors
                                  EPOCH-A 185 B 407a7c8f… · EPOCH-B 414 B 6858b71d… · EPOCH-C = EPOCH-A
2 authority_basis_record_digest   record minus record_digest; stored value MUST reproduce
                                  BASIS-AUTH-1 431 B 7ad84cfb… · BASIS-ADM-1 371 B bf29f3d7…
3 authority_decision_digest       decision minus authority_decision_digest
4 envelope_digest                 envelope minus envelope_digest; closed schema
5 consumer_validation_digest      checks[] frozen order 1..16; evaluated_at participates
6 reliance_record_digest          both authority moments participate
7 integration_package_digest      package minus package_digest; members {path,bytes,sha256}
8 synthetic_profile_digest        profile minus profile_digest
                                  PROFILE-PRODUCER-1 398 B 1c7ac979… · PROFILE-CONSUMER-1 416 B 889ab97b…
```

Their full definitions and complete vector inputs remain as published in v0.4 §2,
§3 and §9, unaltered.

## 3. CLASS 9 — `criterion_observation_digest`

```
domain    one CriterionObservation as defined by projection v0.2 §1–§2 over the
          per-criterion model of projection v0.1
excluded  observation_digest
rule      observation_digest = sha256(canonical(CriterionObservation minus observation_digest))
```

**All fields participate**, without exception: execution identity (`execution_id`,
`trace_id`), semantic identity (`semantic_design_sha256`), projection identity
(`criterion_evidence_projection_sha256`), implementation identity
(`implementation_commit`, `implementation_tree`), the universal observation fields,
the criterion-specific observation fields, and `node_accounting`.

`node_accounting` participating in the digest does **not** make it evidence: it is
bound so that a run's bookkeeping cannot be silently swapped, while remaining
`non_load_bearing: true` semantically. Binding a field and relying on it are
different acts.

An observation never contains its own persisted-file sha256 (projection v0.2 §5).

### Reference vector CLASS-9-FIXTURE-1
...
This fixture demonstrates the serialization only. **It is not an observation of
any criterion**, no criterion was executed to produce it, and it must never be
read as evidence that `T-EPOCH-A` or any other criterion ran.
...
```json
{
  ...
  "criterion_evidence_projection_sha256": "00af89d21fda41adaf4a95d5938d3b6fc90666d4ebf902b9c205aa42e7974db2",
...
digest                9da8ca5c7a8f4c3e8f51353eb814b6561d1b3f63613d7e888c80e9932fa42912
```

## 4. CLASS 10 — `criterion_ledger_digest`

```
domain    one CriterionEvidenceLedger as defined by projection v0.2 §3
...
criterion_order[]  the same order, published in projection v0.2 §4
```
...
different digest and is malformed. This is the one place in the slice where the
distinction between key ordering and array ordering is load-bearing.

Each `observations[]` entry contains exactly five fields:

```
{criterion_id, observation_path, persisted_file_bytes, persisted_file_sha256, observation_digest}
```

### Reference vector CLASS-10-FIXTURE-1
...
Again: a serialization fixture. **No criterion was executed**, no observation file
exists behind these identities, and the entries below are synthetic.
...
digest                d7809ab840920536ac848894c62daa3a881d2b33d25c04b3fbf4789360fd622e
```

Every value above is published literally, so the object can be rebuilt without
inference. As a redundant cross-check only — never as a substitute for the
literal table — the fixture values were generated by
...
Projection v0.2 §5 adds the parallel ordering for the criterion layer:
projection frozen → observation files persisted → ledger written binding both
identities per entry.

## 6. Counts and freeze

```
canonical digest classes              10
persisted-file identity rules          2   plus the general persisted-file rule
...
execution that produces it. The standing requirement of v0.4 §11 remains: a vector
that cannot be recomputed from the document alone is not a vector, it is an
assertion.
