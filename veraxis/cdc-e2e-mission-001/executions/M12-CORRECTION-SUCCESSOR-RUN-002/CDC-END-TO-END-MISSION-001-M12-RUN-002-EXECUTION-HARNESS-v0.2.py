"""Governed execution harness for the M12 correction successor, RUN-002 (v0.2).
...
Execution provenance is bound here too. The core validates run metadata
structurally -- it requires the six fields to be present and says nothing about
their values -- so a caller could otherwise choose the run, trace and producer
identity of a measurement after the owner had already reviewed the decision. This
harness fixes those values, derives producer_version from the bytes of the harness
actually executing, and requires the timestamps to describe this observation
rather than an arbitrary chosen moment. That check runs on the invocation path,
immediately before consequence, not only in an earlier shadow pass.
...
RUN_ID = "CDC-E2E-M12-CORRECTION-SUCCESSOR-RUN-002"
TRACE_ID = "CDC-E2E-M12-CORRECTION-SUCCESSOR-TRACE-002"
PRODUCER = "CDC-END-TO-END-MISSION-001-M12-RUN-002-EXECUTION-HARNESS-v0.2"
RUN_METADATA_FIELDS = frozenset(
    {"run_id", "trace_id", "producer", "producer_version", "occurred_at", "recorded_at"}
)
...
RUN_METADATA_HARDENING_QUESTION = "MOVE_CONSEQUENTIAL_RUN_METADATA_BINDING_INTO_CORE_RUNTIME_GATE"
RUN_METADATA_HARDENING_STATUS = "OUTSIDE_FROZEN_RUN_002_SCOPE"
CORE_RUN_METADATA_VALIDATION = "STRUCTURAL_REQUIRED_FIELDS_ONLY"
HARNESS_RUN_METADATA_VALIDATION = "EXACT_SCHEMA_PLUS_BOUNDED_SEMANTICS"
...
def harness_identity(path: pathlib.Path | None = None) -> str:
    """The producer_version of the harness actually executing.

    Computed from this file's own bytes at runtime; a self-referential expected
    digest could not be written into the file it describes.
    """
    target = path or pathlib.Path(__file__)
...
def verify_run_metadata(
    ...
    *,
    ...
    """Bind the execution provenance of this measurement.

    Kept pure so adversarial fixtures can exercise it without approaching the
    result-bearing route.
    """
    observed = datetime.now(UTC) if now is None else now
    expected_producer_version = harness_identity(harness_path)
    failures: list[str] = []

    missing = sorted(RUN_METADATA_FIELDS - metadata.keys())
    unknown = sorted(metadata.keys() - RUN_METADATA_FIELDS)
    if missing:
        failures.append(f"missing_fields={missing}")
    if unknown:
        failures.append(f"unknown_fields={unknown}")
    ...
    for name, expected in (
        ...
        if metadata.get(name) != expected:
            failures.append(f"{name}={metadata.get(name)!r} (required {expected!r})")

    occurred = _parse_utc(metadata.get("occurred_at"))
    recorded = _parse_utc(metadata.get("recorded_at"))
    if occurred is None:
        failures.append(
            f"occurred_at is not an RFC3339 UTC timestamp: {metadata.get('occurred_at')!r}"
        )
    if recorded is None:
        failures.append(
            f"recorded_at is not an RFC3339 UTC timestamp: {metadata.get('recorded_at')!r}"
        )
    if occurred is not None and recorded is not None and occurred != recorded:
        failures.append("occurred_at != recorded_at")
    drift = None
    if occurred is not None:
        drift = abs((observed - occurred).total_seconds())
        if drift > TIMESTAMP_TOLERANCE_SECONDS:
            failures.append(
                f"timestamp is {drift:.0f}s from this observation, tolerance "
    ...
    return {
        ...
        "core_validation": CORE_RUN_METADATA_VALIDATION,
        "harness_validation": HARNESS_RUN_METADATA_VALIDATION,
    }
...
def preflight() -> dict[str, Any]:
    ...
    return {
        ...
        "core_run_metadata_validation": CORE_RUN_METADATA_VALIDATION,
        "run_002_harness_run_metadata_validation": HARNESS_RUN_METADATA_VALIDATION,
        "run_metadata_hardening_question": RUN_METADATA_HARDENING_QUESTION,
        "run_metadata_hardening_status": RUN_METADATA_HARDENING_STATUS,
        "run_metadata_expected_producer_version": harness_identity(),
        "run_metadata_verified_on_invocation_path": True,
...
def execute(run_metadata: dict[str, str]) -> mission.CorrectionSuccessorResult:
    ...
    Every gate is re-run here, including source identity and the exact run
    metadata, so neither a stale preflight nor a caller-chosen provenance can
    authorize this. Invoked exactly once by the caller; there is no loop, retry or
    fallback path.
    """
    report = preflight()
    ...
    metadata = verify_run_metadata(run_metadata)
    if not metadata["accepted"]:
        raise HarnessRefusalError(f"run metadata refused: {metadata['failures']}")
