...
def test_operator_guide_states_the_candidate_authority_boundary(documents: dict[str, str]) -> None:
    ...
    plain = text.replace("*", "")
    assert "candidate material only" in plain
    assert "no authority" in plain
    assert "admission rights" in plain
...
def test_claims_bearing_documents_are_unchanged_by_this_work_order(repo_root: Path) -> None:
    ...
    assert (repo_root / "docs/decisions/OIC-OWNER-DECISION-004.md").is_file()
