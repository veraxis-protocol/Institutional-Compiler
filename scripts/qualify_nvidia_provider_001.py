...
Provider-only qualification gate for the exact NVIDIA NIM path used by bounded OIC
experiments. This tests transport availability, structured-response compatibility,
and latency headroom. It does not test institutional semantics, canonicalization,
or Institutional IR.
"""
...
WORK_ORDER: Final[str] = "OIC-NVIDIA-PROVIDER-QUALIFICATION-001"
BASE_SHA: Final[str] = "c4a87eb8483c3bd965612b601399463e005bd73e"
PLAN_PATH = ROOT / "benchmarks/provider-qualification/nvidia-nim-001/PLAN-v0.1.json"
FREEZE_PATH = ROOT / "benchmarks/provider-qualification/nvidia-nim-001/PLAN-FREEZE-v0.1.json"
RECEIPT_PATH = (
    ROOT / ".local/provider-qualification-receipts" / "OIC-NVIDIA-PROVIDER-QUALIFICATION-001.json"
)
...
def preflight() -> dict[str, Any]:
    ...
    if sha256(PLAN_PATH) != freeze["plan_sha256"]:
        raise SystemExit("FAIL provider-qualification plan digest mismatch")
    plan: dict[str, Any] = json.loads(PLAN_PATH.read_text(encoding="utf-8"))
    if plan["work_order"] != WORK_ORDER or plan["base_sha"] != BASE_SHA:
        raise SystemExit("FAIL qualification identity mismatch")
    if plan["provider"] != {
...
def execute_live() -> tuple[list[dict[str, Any]], str]:
    if RECEIPT_PATH.exists():
        raise SystemExit(f"STOP qualification receipt already exists: {RECEIPT_PATH}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    plan = preflight()
    print(f"PASS frozen provider-qualification plan verified; {len(PROBES)} probes")
    ...
    authorized = "YES" if disposition == "QUALIFIED" else "NO"
    print(f"semantic successor authorized: {authorized}")
    return 0
