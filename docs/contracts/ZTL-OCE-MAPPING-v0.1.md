...
| WP-3 | 2 | matched classification row = 28, epistemic_status = CONDITIONALLY_SUPPORTED, warranty_grade_observed = sound, grade sufficient, unverified_ground_policy = allow_with_disclosure | PRESERVE | ALLOW | CONTROL_REQUIREMENT | OIC-D-0005 |
...
| SC-RD-005 | OVERLAY_COMPLETENESS | every RuntimeDecision | applied_control_overlay_ids must equal, exactly and in order, the overlays derivable from the envelope and kernel result: the applicable stage-2 WP rule where the classification row declares stage_2_applies, then the applicable non-identity stage-3 DM overlay. | completeness relative to the envelope and kernel result |
| SC-RD-006 | CONDITIONAL_ALLOW_GRADE | RuntimeDecision where epistemic_status = CONDITIONALLY_SUPPORTED and execution_disposition = ALLOW | warranty_grade_observed must equal sound; the matched classification row must be 28; WP-3 must appear in applied_control_overlay_ids; OIC-D-0005 and OIC-W-0015 must both be retained in reason_codes; and every subscription requirement (SC-RD-001, SC-RD-002) must independently pass. OIC-W-0025 (the instability code for row 29) must never appear on a decision reaching this ALLOW. | binding a value constraint to a specific classification row |
| SC-WA-001 | GROUND_PARTITION | WarrantArtifact produced under kernel_profile_id ztl-v0.1 | dependency_ids are exactly the formula atoms marked T or F; unverified_ground_ids are exactly the atoms marked Z; the two arrays are disjoint; their union equals the EVALUATED FORMULA's atom set. The partition is over the formula's atom set, not over the caller's marking: an atom present in the marking but absent from the formula belongs to neither array, and an atom present in the formula but absent from the marking defaults to Z and belongs in unverified_ground_ids. Neither array claims minimality. | a partition derived from the evaluated marking |
| SC-WA-002 | HASH_PROJECTIONS | WarrantArtifact produced under kernel_profile_id ztl-v0.1 | Both digests are RECOMPUTED and compared, not merely required to be present. formula_hash = SHA-384 over the UTF-8 bytes of the kernel-rendered formula, serialised sha384:<96 lowercase hex>. output_hash = SHA-256 over the semantic projection {kernel_rendered_formula, disposition, raw_verdict, warranty_grade, unverified_ground_ids}, serialised UTF-8 with sorted keys, compact separators ',' and ':', no ASCII escaping and no whitespace, as sha256:<64 lowercase hex>. why, marking, dependency_ids, generated_at and every OIC-enriched field are excluded. | the provenance of a digest's input |
